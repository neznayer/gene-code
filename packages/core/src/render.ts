...
export function renderNode(node: LayoutNode): string {
  switch (node.type) {
    ...
    case "rect":
      return `<rect x="${node.x}" y="${node.y}" width="${node.width}" height="${node.height}" fill="lightgray" stroke="black" />`;
    ...
    case "circle":
      return `<circle cx="${node.x}" cy="${node.y}" r="${node.radius}" fill="${node.color}" />`;
    case "group":
      return `<g transform="translate(${node.x},${node.y})">
